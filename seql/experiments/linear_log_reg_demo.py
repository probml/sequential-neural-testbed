...
from seql.environments.base import make_random_linear_classification_environment, make_random_poly_classification_environment
...
def callback_fn(agent, env, agent_name, **kwargs):
    ...
    for i in range(10):
        ...
        pred = model_fn(w.reshape((2,2)), grid)
...
def main():
    ...
    obs_noise = 0.
    env = lambda batch_size: make_random_linear_classification_environment(env_key,
                                                                         nfeatures,
                                                                         ntrain,
                                                                         ntest,
                                                                         obs_noise=obs_noise,
    ...
    def eekf_model_fn(params, x):
        return model_fn(params.reshape((nfeatures, nclasses)), x)
