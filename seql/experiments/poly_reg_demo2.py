...
def main():
    ...
    ns = jnp.arange(100, 800, 100)
    ntest = 1000
    nsamples_input, nsamples_output = 1, 1
    test_batch_size = 1

    fig, axes = plt.subplots(nrows=2, ncols=1)
    for njoint, ax in zip(taus, axes.flatten()):
        for rng_key, n  in zip(random.split(key, len(ns)), ns):
            env_key, train_key = random.split(rng_key)
            nsteps = min(10, n)
            train_batch_size = n // nsteps
        ...
        ax.plot(ns, y, '-o', c=colors[njoint])
        ax.set_xticks(ns)
        ax.set_xlabel("Number of Training Points")
        ax.set_ylabel("Average KL Estimate")
        ax.set_title(f"Njoint: {njoint}")

    plt.tight_layout()
    plt.savefig("njoint.png")
